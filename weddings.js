import { collection, doc, getDoc, getDocs, initFirebase, isFirebaseConfigured, onAuthStateChanged, onSnapshot, query, serverTimestamp, setDoc, signOut, updateDoc, where, writeBatch, httpsCallable } from "./firebase-config.js";

const state = { services: null, user: null, weddings: [], filter: "all", editId: "", deleteId: "", unsubscribe: null, unsubscribeAuth: null, initialized: false, snapshotGeneration: 0 };
...
function init() {
  ...
  document.addEventListener("click", (event) => { if (!event.target.closest(".menu-wrap")) closeActionMenus(); });
  document.addEventListener("keydown", (event) => { if (event.key === "Escape") closeActionMenus(); });
  state.unsubscribeAuth?.();
  state.unsubscribeAuth = onAuthStateChanged(state.services.auth, (user) => { if (!user) return location.replace("./dashboard-login.html?message=session-required"); state.user = user; el.userEmail.textContent = user.email || "Planner"; el.loadingState.hidden = true; el.workspace.hidden = false; watchWorkspace(); });
...
function watchWorkspace() {
  ...
  state.unsubscribe = onSnapshot(access, async (snapshot) => {
    const accessEntries = snapshot.docs.map((row) => ({ ...row.data(), id: row.id }));
    ...
    const [indexedWeddings, legacyWeddings] = await Promise.all([
      loadIndexedWeddings(accessEntries),
      loadLegacyActiveWeddings()
    ]);
    if (generation !== state.snapshotGeneration) return;
    // Both sources are resolved from top-level wedding documents, then merged
    // by Firestore ID. The access index can never render stale card metadata.
    state.weddings = replaceRowsById([...legacyWeddings, ...indexedWeddings]);
    ...
    if (error.code === "permission-denied") {
      const legacyWeddings = await loadLegacyActiveWeddings();
      if (generation !== state.snapshotGeneration) return;
      state.weddings = replaceRowsById(legacyWeddings);
      await enrichStats();
      render();
...
async function loadIndexedWeddings(accessEntries) {
  const weddings = await Promise.all(accessEntries.map(async (entry) => {
    try {
      const weddingDoc = await getDoc(doc(state.services.db, "weddings", entry.id));
      return weddingDoc.exists() ? { ...weddingDoc.data(), id: weddingDoc.id } : null;
    } catch (error) {
      console.warn(`Wedding access entry ${entry.id} could not be resolved.`, error);
      return null;
    }
  }));
  return weddings.filter(Boolean);
}
...
async function loadLegacyActiveWeddings() {
  try {
    ...
    const candidates = snapshot.docs.map((row) => ({ ...row.data(), id: row.id, legacy: true }));
    const permissions = await Promise.all(
      candidates.map((wedding) =>
        getDoc(
          doc(
            state.services.db,
            "weddings",
    ...
    return candidates.filter(
      (_, index) =>
        permissions[index].exists() &&
        permissions[index].data().canViewDashboard === true,
    );
  } catch (error) {
    console.error("Accessible weddings could not be loaded.", error);
    return [];
  }
}

function replaceRowsById(rows) {
  const byId = new Map();
  rows.forEach((row) => {
...
function card(w) {
  ...
  const ownerActions = isOwner ? `<div class="menu-wrap"><button data-action="menu" data-id="${w.id}" type="button" aria-label="More actions" aria-expanded="false" aria-controls="menu-${w.id}">More</button><div class="action-menu" id="menu-${w.id}" hidden><button data-action="edit" data-id="${w.id}" type="button">Edit details</button><button data-action="duplicate" data-id="${w.id}" type="button">Duplicate setup</button><button data-action="archive" data-id="${w.id}" type="button">${w.status === "archived" ? "Restore wedding" : "Archive wedding"}</button><button class="menu-delete" data-action="delete" data-id="${w.id}" type="button">Delete wedding</button></div></div>` : "";
...
async function handleCardAction(event) {
  ...
  if (button.dataset.action === "menu") { const menu = document.getElementById(`menu-${wedding.id}`); const shouldOpen = menu.hidden; closeActionMenus(menu); menu.hidden = !shouldOpen; button.setAttribute("aria-expanded", String(shouldOpen)); return; }
